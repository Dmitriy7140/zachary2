...
Куб: `game/cube_catalog.py` содержит стабильные ключи механик и замороженные `LayoutPolicy`, `game/cube.py` — детерминированный генератор 4×4 и scheduler, `content/cube.py` — тексты под ключами каталога, `handlers/cube.py` — private-only интерфейс и реестр экранов, `scripts/cube_map_renderer.py` — независимый Pillow-рендерер личной карты. Поколение глобально для всех игроков; authoritative комнаты, проходы, раскрытия, ловушки, позиции, личные посещения, текущее соседство игроков, idempotency-запросы, waitlist и lease-outbox находятся в SQLite. Вход/ход/победа и запись личного посещения выполняются одной economy-транзакцией, а Telegram и проверка Густава идут после commit. Семантику persisted-ключа и ordered pools старой layout-версии не меняй: новая механика получает новый ключ и новую `LayoutPolicy`. Таймеры, цена, приз на участника и лимит задаются `CUBE_*` в `config.py`/`.env.example`.
...
5. **Долгие ожидания — абсолютный ISO timestamp в SQLite + scheduler.** Не клади рынок, улов, долг, рейд, тираж лотереи или бизнес-таймер в FSM/RAM. В частности, у слизней каждая единица готовки хранится отдельной строкой `slug_cooking` со статусом `cooking` / `ready` / `delivered`; планировщик атомарно выдаёт готовое только после срока. Scheduler-поля используют naive `datetime.now().isoformat()` в локальном времени, а schema defaults `datetime('now')` — UTC. Сохраняй представление конкретной колонки; не смешивай aware/naive или local/UTC без общей миграции.
