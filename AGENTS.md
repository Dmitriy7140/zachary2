...
- `main.py` — composition root: инициализирует storage, Bot/Dispatcher, вручную подключает routers в значимом порядке, запускает polling и десять scheduler-задач.
...
1. **SQLite переживает рестарт.** `storage.init()` открывает общий `aiosqlite.Connection` и отдельное сериализованное economy-соединение для критичных списаний/лотереи. В SQLite живут профили (включая одноразовый уровень палеты рынка), деньги, опыт, inventory, cooldown/status, статистика, рынок, ставки, долги, рыбалка, несколько бизнесов игрока, отмыв с ключом компании, поштучная очередь `slug_cooking`, тиражи/билеты/outbox лотереи и `meta`.
...
- Почти каждый mutating helper сам делает `commit()`. Несколько helper-вызовов не образуют транзакцию и могут перемежаться конкурентными update. `spend_zbucks[_traced]`, активация прятки, лотерея, атомарные бизнес-/рыночные операции, `add_item` и `remove_item` сериализованы через economy connection + `BEGIN IMMEDIATE`; `take_stock` остаётся read-check-write и сам по себе не concurrency-safe.
